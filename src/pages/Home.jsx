...
const LetterSection = () => (
  <div className="px-6 md:px-12 w-full max-w-6xl mx-auto">
    ...
    />
    {/* Letter Content */}
    <p className="mt-6 text-gray-700 text-sm text-justify">    <strong>Esteemed Delegates,</strong>
    <br />
    <br />
...
const FullImage = ({ src }) => {
  ...
  return (
    <div className="w-full h-screen overflow-hidden rounded-t-[7%]">
      <motion.img src={src} alt="Full View" className="w-full h-full object-cover" style={{ scale }} />
    </div>
  );
};

// ✅ Delegate Page Component
const DelegatePage = () => {
  return (
...
const Home = () => {
  ...
  const sections = [
    { id: 1, content: <HomeScreen />, classname: "h-auto md:h-screen bg-cover bg-center", style: { backgroundImage: "url('/mahe2.png')" } },
    { id: 2, content: <LetterSection />, classname: "rounded-t-[7%] bg-gray-200 p-4 sm:p-8" },
    { id: 3, content: <LetterSectionGenSec />, classname: "rounded-t-[7%] bg-gray-200 p-4 sm:p-8" },
    { id: 4, content: <ConferenceHighlights />, classname: "rounded-t-[7%] bg-black h-auto md:h-screen p-4 sm:p-8", idName: "highlights" },
    { id: 5, content: <MunStats />, classname: "rounded-t-[7%] bg-[#f2eaea] h-auto md:h-screen p-4 sm:p-8", idName: "stats" },
    { id: 6, content: <FullImage src="/mun.jpg" />, classname: "rounded-t-[7%] h-auto md:h-screen" },
    { id: 7, content: <DelegatePage />, classname: "" },
  ];
  const [isPopupOpen, setIsPopupOpen] = useState(false);

  useEffect(() => {
    setIsPopupOpen(true); // Show popup on page load
  }, []);
  ...
  return (
    <div className="min-h-screen max-w-full bg-gradient-to-b from-gray-50 to-white">
      ...
      {/* Content Sections */}
      <ul className="list-none p-0 grid grid-cols-1 gap-0 pb-[45vh] mt-[70px]">
        {sections.map((section, index) => {
          let translateY;
          switch (index) {
            case 0: translateY = homeY; break;
            case 1: translateY = letterY; break;
            case 2: translateY = letterY2; break;
            case 3: translateY = confY; break;
            case 4: translateY = statsY; break;
            case 5: translateY = imageY; break;
            case 6: translateY = delegateY; break;
            default: translateY = "0vh";
          }
          return (
            <motion.li key={section.id} id={section.idName} className="sticky top-0 flex justify-center" style={{ y: translateY }}>
              <div style={section.style} className={`shadow-xl flex flex-col w-full ${section.classname} text-black`}>
                {section.content}
              </div>
            </motion.li>
          );
